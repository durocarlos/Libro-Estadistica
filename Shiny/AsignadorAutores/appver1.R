# ======================================================
# app.R — Asignador de Autores + Reportes + Correos (libre) + Riesgos PMP
# (versión completa con reporte integral de coautores y exportación PDF robusta)
# ======================================================
                       ...
                       fluidRow(
                         ...
                         column(9,
                                tabsetPanel(id = "report_tabs",
                                            ...
                                            # ============= NUEVO: REPORTE INTEGRAL PDF (coautores visibles) =============
                                            tabPanel("Autores y coautores (PDF)",
                                                     br(),
                                                     div(class="d-flex gap-2 flex-wrap",
                                                         downloadButton("repint_pdf_dl", "📄 Descargar PDF", class = "btn-danger"),
                                                         downloadButton("repint_csv_dl", "⬇️ CSV", class = "btn-success")
                                                     ),
                                                     br(),
                                                     DTOutput("repint_tbl")
                                            ),
                                            # ============= NUEVO: RIESGOS PMP (se conserva) =============
                                            tabPanel("Riesgos PMP",
                                                     ...
                                                     br(),
                                                     bslib::card(
                                                       bslib::card_header("Acciones (semáforo)"),
                                                       DTOutput("pmp_tbl_acciones")
                                                     ),
                                                     bslib::card(
                                                       bslib::card_header("Heatmap Prob × Impacto"),
                                                       plotOutput("pmp_heatmap", height = 420)
                                                     ),
                                                     bslib::card(
                                                       bslib::card_header("Eventos recientes"),
                                                       DTOutput("pmp_tbl_eventos")
                                                     ),
                                                     bslib::card(
                                                       bslib::card_header("Exportar / Guardar"),
                                                       textInput("pmp_rep_name", "Nombre base",
...
server <- function(input, output, session){
  ...
  # Asegura listas largas de coautores sin truncar
  ...
  observeEvent(input$up_cap,   { s<-input$tabla_cap_rows_selected; req(length(s)==1); df<-cap_df(); exec(con,"UPDATE capitulo_autor SET orden=IFNULL(orden,10000) WHERE capitulo_id=? AND autor_id=?", params=list(df$capitulo_id[s], df$autor_id[s])); normalizar_cap(con, df$capitulo_id[s]); showNotification("Movido ↑ en capítulo", type="message") })
  observeEvent(input$down_cap, { s<-input$tabla_cap_rows_selected; req(length(s)==1); df<-cap_df(); exec(con,"UPDATE capitulo_autor SET orden=IFNULL(orden,10000) WHERE capitulo_id=? AND autor_id=?", params=list(df$capitulo_id[s], df$autor_id[s])); normalizar_cap(con, df$capitulo_id[s]); showNotification("Movido ↓ en capítulo", type="message") })
  observeEvent(input$up_sub,   { s<-input$tabla_sub_rows_selected; req(length(s)==1); df<-sub_df(); exec(con,"UPDATE subcapitulo_autor SET orden=IFNULL(orden,10000) WHERE subcapitulo_id=? AND autor_id=?", params=list(df$subcapitulo_id[s], df$autor_id[s])); normalizar_sub(con, df$subcapitulo_id[s]); showNotification("Movido ↑ en subcapítulo", type="message") })
  observeEvent(input$down_sub, { s<-input$tabla_sub_rows_selected; req(length(s)==1); df<-sub_df(); exec(con,"UPDATE subcapitulo_autor SET orden=IFNULL(orden,10000) WHERE subcapitulo_id=? AND autor_id=?", params=list(df$subcapitulo_id[s], df$autor_id[s])); normalizar_sub(con, df$subcapitulo_id[s]); showNotification("Movido ↓ en subcapítulo", type="message") })
  ...
  # ===================== NUEVO REPORTE INTEGRAL (coautores visibles) =====================
  repint_data <- reactive({
    ...
    df <- q(con, "
      SELECT
        ...
        c.titulo,
        GROUP_CONCAT(
          CASE WHEN ca.rol_id = 1 THEN a.nombre_completo END
          ORDER BY COALESCE(ca.orden, 9999), a.nombre_completo
          SEPARATOR ' / '
        ) AS autor_principal,
        GROUP_CONCAT(
          CASE WHEN ca.rol_id = ? THEN a.nombre_completo END
          ORDER BY COALESCE(ca.orden, 9999), a.nombre_completo
          SEPARATOR '; '
        ) AS coautores,
    ...
    df$con_coautores <- df$n_coautores > 0
    if (!is.null(input$r_caps) && length(input$r_caps) > 0) df <- df %>% filter(capitulo_id %in% input$r_caps)
    df
  })
  output$repint_tbl <- DT::renderDT({
    df <- repint_data(); validate(need(nrow(df) > 0, "Sin datos"))
  ...
  output$repint_csv_dl <- downloadHandler(
    filename = function() sprintf("reporte_integral_%s.csv", format(Sys.Date(), "%Y%m%d")),
    content  = function(file){
      repint_data() %>% transmute(capitulo_id, numero, titulo, autor_principal, coautores,
                                  n_principal, n_coautores, total_autores) %>% arrange(numero) %>%
        readr::write_csv(file)
    }
  )
  # Generación PDF robusta (LaTeX -> pagedown)
  make_pdf_integral <- function(tbl, kpi, out_pdf){
    # Intento 1: PDF vía LaTeX
    rmd <- tempfile(fileext = ".Rmd")
    writeLines(con = rmd, text = paste0(
      "---\n",
      "title: \"Autores y coautores por capítulo\"\n",
      ...
      "```\n\n",
      "## Resumen\n\n",
      "- Capítulos: `r params$kpi$total_caps`\n",
      "- Con autor principal: `r params$kpi$con_principal`\n",
      "- Sin autor principal: `r params$kpi$sin_principal`\n",
      "- Con coautores: `r params$kpi$con_coautores`\n",
      "- Coautores promedio por capítulo: `r params$kpi$prom_coautores`\n\n",
    ...
    if (latex_ok) {
      try(rmarkdown::render(input=rmd, output_file=out_pdf, output_dir=dirname(out_pdf),
                            params=list(kpi=kpi, tabla=tbl), envir=new.env(parent=globalenv()), quiet=TRUE), silent=TRUE)
      if (file.exists(out_pdf)) return(invisible(TRUE))
    }
    # Intento 2: HTML + pagedown
    if (requireNamespace("pagedown", quietly = TRUE)) {
      ...
      writeLines(con = html_rmd, text = paste0(
        ...
        "params: {kpi: !r NULL, tabla: !r NULL}\n",
        "---\n\n",
        "## Resumen\n\n",
        "- Capítulos: `r params$kpi$total_caps`\n",
        "- Con autor principal: `r params$kpi$con_principal`\n",
  ...
  output$repint_pdf_dl <- downloadHandler(
    filename    = function() sprintf("Autores_y_coautores_%s.pdf", format(Sys.Date(), "%Y-%m-%d")),
    contentType = "application/pdf",
    content = function(file){
      df <- repint_data()
      kpi <- list(
        total_caps     = nrow(df),
        con_principal  = sum(df$n_principal > 0, na.rm=TRUE),
        sin_principal  = sum(df$n_principal == 0, na.rm=TRUE),
        con_coautores  = sum(df$con_coautores, na.rm=TRUE),
        prom_coautores = if (nrow(df)) round(mean(df$n_coautores, na.rm=TRUE), 2) else 0
      )
      tbl <- df %>% transmute(Capítulo=numero, Título=titulo, `Autor principal`=autor_principal,
                              Coautores=coautores, `Total autores`=total_autores) %>% arrange(Capítulo)
      ok <- make_pdf_integral(tbl, kpi, file)
      if (!ok) showNotification("No hay LaTeX ni pagedown disponibles: instala tinytex::install_tinytex() o pagedown.", type="error", duration=8)
    }
  )
  ...
  output$pmp_conn_status <- renderUI({
    ok <- TRUE
    tryCatch(DBI::dbGetQuery(con, "SELECT 1"), error = function(e) ok <<- FALSE)
  ...
  output$kpi_extreme <- renderText({
    df <- pmp_prior(); n <- df %>% dplyr::filter(prioridad %in% c("Extreme","Crítica","Critica")) %>% dplyr::summarise(n=sum(total,na.rm=TRUE)) %>% dplyr::pull(n)
    format(n %||% 0)
  })
  output$kpi_high   <- renderText({ df<-pmp_prior(); n<-df %>% dplyr::filter(prioridad %in% c("High","Alta"))   %>% dplyr::summarise(n=sum(total,na.rm=TRUE)) %>% dplyr::pull(n); format(n %||% 0) })
  output$kpi_medium <- renderText({ df<-pmp_prior(); n<-df %>% dplyr::filter(prioridad %in% c("Medium","Media"))%>% dplyr::summarise(n=sum(total,na.rm=TRUE)) %>% dplyr::pull(n); format(n %||% 0) })
  output$kpi_low    <- renderText({ df<-pmp_prior(); n<-df %>% dplyr::filter(prioridad %in% c("Low","Baja"))   %>% dplyr::summarise(n=sum(total,na.rm=TRUE)) %>% dplyr::pull(n); format(n %||% 0) })
  output$pmp_tbl_acciones <- DT::renderDT({
    dat <- pmp_acc()
    validate(need(ncol(dat)>0, "Sin datos (¿existe v_riesgo_acciones?)."))
  ...
  output$pmp_heatmap <- renderPlot({
    h <- pmp_heat()
    validate(need(ncol(h)>0, "Sin datos (¿existe v_riesgo_heatmap?)."))
  ...
  output$pmp_tbl_eventos <- DT::renderDT({
    e <- pmp_evt()
    validate(need(ncol(e)>0, "Sin datos (¿hay registros en riesgo_evento?)."))
    ...
    if (tab == "Autores") {
      ...
      div(class="well", h4("Leyenda rápida — Reportes"),
          tags$ul(
            ...
            tags$li(strong("Reporte integral (PDF):")," autor principal + coautores por capítulo."),
